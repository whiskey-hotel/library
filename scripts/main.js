...
class book {
	...
	static bookCount(obj) {
		return obj.length;
	}

	static readBookCount(obj) {
		let count = 0;
		obj.forEach((index) => {
			if (index.read) {
				count += 1;
	...
	removeBook = function (dataAttribute, el, childEl) {
		const index = parseInt(dataAttribute);
		if (index > -1) {
			book.myLibrary.splice(index, 1);
			el.removeChild(childEl);
		}
		totalBooks.textContent = book.bookCount(book.myLibrary);
		readBooks.textContent = book.readBookCount(book.myLibrary);
		book.dataAttributeUpdate();
	};
...
newBookForm.addEventListener("submit", function () {
	...
	newBook.addBooks();
	newBook.display(book.myLibrary.length - 1);
	newBookForm.reset();
	totalBooks.textContent = book.bookCount(book.myLibrary);
});
